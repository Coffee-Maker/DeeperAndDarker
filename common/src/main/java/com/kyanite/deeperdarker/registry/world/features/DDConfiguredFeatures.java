...
public class DDConfiguredFeatures {
                                            ...
                                            new SimpleBlockConfiguration(
                                                    new WeightedStateProvider(
                                                            SimpleWeightedRandomList.<BlockState>builder()
                                                                    .add(DDBlocks.GLOOMY_GRASS.get().defaultBlockState(), 50)
                                                                    .add(DDBlocks.GLOOM_CACTUS.get().defaultBlockState(), 35)
                                                                    .add(DDBlocks.SCULK_TENDRILS.get().defaultBlockState(), 16)
                                                    )
                                            )
                                    )
                            )
    ...
    public static final Supplier<ConfiguredFeature<?, ?>> GLOOM_SCULK_VEGETATION_COLLECTION =
            RegistryHelper.registerConfiguredFeature("gloom_sculk_bonemeal",
                    () -> new ConfiguredFeature<>(Feature.VEGETATION_PATCH,
                            new VegetationPatchConfiguration(
                                    DDTags.Blocks.GLOOM_SCULK_REPLACEABLE,
                                    BlockStateProvider.simple(DDBlocks.GLOOM_SCULK.get()),
                                    PlacementUtils.inlinePlaced(Holder.direct(GLOOM_SCULK_VEGETATION_BASE.get())),
                                    CaveSurface.FLOOR,
                                    ConstantInt.of(1), 0, 2, 0.3f,
                                    UniformInt.of(1, 2), 0.7f
                            )
                    )
            );
